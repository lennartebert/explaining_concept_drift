...
class KSTestPC(PopulationComparer):
    ...
    def compare(self, population_1, population_2):
        ...
        pop_1_no_nan = [
            sample for sample in population_1 if sample is not None]
        pop_2_no_nan = [
            sample for sample in population_2 if sample is not None]
        ...
        test_statistics, p_value = scipy.stats.kstest(
            pop_1_no_nan, pop_2_no_nan)
        return p_value
...
class HotellingsTSquaredPC(PopulationComparer):
    ...
    def compare(self, population_1, population_2):
        ...
        try:
            ...
            t_squared = (
                nx*ny)/(nx+ny) * np.matmul(np.matmul(delta.transpose(), np.linalg.inv(S_pooled)), delta)
...
class HellingerDistancePC(PopulationComparer):
    ...
    def compare(self, population_1, population_2):
        ...
        """
        p, q = preprocess_get_normalized_contingency_table(
            population_1, population_2)

        hellinger_distance = np.sqrt(
            np.sum((np.sqrt(p) - np.sqrt(q)) ** 2)) / np.sqrt(2)
...
class ChiSquaredPC(PopulationComparer):
    ...
    def compare(self, population_1, population_2):
        ...
        stat, p_value, dof, expected = scipy.stats.chi2_contingency(
            contingency_table)
